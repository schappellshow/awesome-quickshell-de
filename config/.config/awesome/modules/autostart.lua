...
-- No screenshot daemon here. The Print key runs `spectacle -r`, which keeps
-- its own editor window open after Copy — that window is what holds the X11
-- clipboard selection, so nothing needs to linger in the background between
-- screenshots. (flameshot used to need a resident instance for exactly this
-- reason; see modules/keys.lua.)
